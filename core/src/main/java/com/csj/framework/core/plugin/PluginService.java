package com.csj.framework.core.plugin;

import com.csj.framework.plugin.Plugin; // From plugin-api
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.beans.factory.annotation.Value;
// import org.springframework.context.ApplicationContext; // If passing context to plugins
// import org.springframework.beans.factory.annotation.Autowired; // If passing context
import org.springframework.stereotype.Service;

import jakarta.annotation.PostConstruct; // For @PostConstruct
import jakarta.annotation.PreDestroy;  // For @PreDestroy
...
public class PluginService {
    ...
    @Value("${framework.plugin.directory:./plugins}") // Default to a 'plugins' subdirectory
    private String pluginDirectoryPath;

    // @Autowired
    // private ApplicationContext applicationContext; // If plugins need context
    ...
    public void loadPlugins() {
        ...
        for (File pluginFile : pluginFiles) {
            try {
                ...
                // Create a new classloader for each plugin JAR for better isolation,
                // though true isolation is much harder (requires parent last or custom classloaders).
                // For ServiceLoader to work across plugins loaded this way, the Plugin API
                // must be accessible from the parent classloader (application's classloader).
                URLClassLoader pluginClassLoader = new URLClassLoader(new URL[]{pluginUrl}, getClass().getClassLoader());
                pluginClassLoaders.add(pluginClassLoader); // Keep track to close it later

                // Using ServiceLoader to find implementations of the Plugin interface
                ...
                for (Plugin plugin : serviceLoader) {
                    ...
                    // plugin.initialize(applicationContext); // If initializing with context
                    plugin.load();
    ...
    public Map<String, Plugin> getLoadedPlugins() {
        return new ConcurrentHashMap<>(loadedPlugins); // Return a copy
    }
